...
class CreateStubGenerator
{
    ...
    {
        $stub           = Stub::create('create.stub', 'resources/views/' . Str::plural(strtolower($this->name)));
        $this->contents = $stub->getContents();
        $this->replace('model', ucfirst($this->name));

        $columns = '<form  class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="post" action="{{route(\'' . Str::plural(strtolower($this->name)) . '.store\'' . ')}}">';
        $columns .= '@csrf';
        foreach ($this->columns as $column) {
            $columns .= '<div class="mb-4">';
            $columns .= '<label class="block text-gray-700 text-sm font-bold mb-2">' . ucfirst($column) . '</label>';
            $columns .= '<input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" name="' . $column . '" type="text" placeholder="Enter ' . ucfirst($this->name) . '">';
            $columns .= '</div>' . PHP_EOL;
        }
